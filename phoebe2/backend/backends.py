...
from phoebe2.parameters import ParameterSet
from phoebe2 import dynamics
from phoebe2.backend import mesh, etvs
from phoebe2.distortions  import roche
from phoebe2.frontend import io
from phoebe2 import u, c
...
def _extract_from_bundle_by_time(b, compute, store_mesh=False, time=None):
    ...
    :parameter b: the :class:`phoebe2.frontend.bundle.Bundle`
...
def _create_syns(b, needed_syns, store_mesh=False):
    ...
    :return: :class:`phoebe2.parameters.parameters.ParameterSet` of all new parameters
    """
...
def phoebe(b, compute, time=[], as_generator=False, **kwargs):
    ...
    * Compute:
        * all parameters in :func:`phoebe2.parameters.compute.phoebe`
    * Orbit:
        * TOOD: list these
    * Star:
        * TODO: list these
    ...
    * rv (dynamical only):
    ...
    This function will almost always be called through the bundle, using
        * :meth:`phoebe2.frontend.bundle.Bundle.add_compute`
        * :meth:`phoebe2.frontend.bundle.Bundle.run_compute`

    :parameter b: the :class:`phoebe2.frontend.bundle.Bundle` containing the system
        and datasets
    ...
    :return: a list of new synthetic :class:`phoebe2.parameters.parameters.ParameterSet`s
    """
...
def legacy(b, compute, time=[], **kwargs): #, **kwargs):#(b, compute, **kwargs):
    ...
    * Compute:
        * all parameters in :func:`phoebe2.parameters.compute.legacy`
    * Orbit:
        * TOOD: list these
    * Star:
        * TODO: list these
    ...
    * rv (dynamical only):
    ...
    build up keys for each phoebe1 parameter, so that they correspond to the cor    rect phoebe2 parameter. Since many values of x, y, and z it is unecessary to    make specific dictionary items for them. This function does this and also ma    kes sure the correct component is being called.

    Args:
        key: The root of the key word used in phoebe1: component: the name of th        e phoebe2 component associated with the given mesh.

    Returns:
        new key(s) which are component specific and vector direction specific (x        ,y, z) for phoebe1 and phoebe2.
    ...
    def ret_dict(key):
        """
        Build up dictionary for each phoebe1 parameter, so that they correspond         to the correct phoebe2 parameter.
        Args:
            key: The root of the key word used in phoebe1: component: the name o            f the phoebe2 component associated with the given mesh.

        Returns:
            dictionary of values which should be unique to a single parameter in            phoebe 2.
...
def photodynam(b, compute, time=[], **kwargs):
    ...
    * Compute:
        - all parameters in :func:`phoebe2.parameters.compute.photodynam`
...
def jktebop(b, compute, time=[], **kwargs):
    ...
    * Compute:
        -  all parameters in :func:`phoebe2.parameters.compute.jktebop`
