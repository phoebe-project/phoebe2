...
class TestInterpQuery:
    ...
    def test_col_index(self):
        ...
        assert query.col_index('teffs') == 0
        assert query.col_index('loggs') == 1
        assert query.col_index('abuns') == 2
    ...
    def test_contiguous_array_conversion(self):
        ...
        pts_non_contiguous = pts_orig.T.T  # Create non-contiguous array

        # Verify it's non-contiguous
        assert not pts_non_contiguous.flags.c_contiguous
