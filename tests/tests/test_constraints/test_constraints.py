...
def test_esinw_ecosw(verbose=False):
    if verbose:
        print("b = phoebe.default_binary()")
    b = phoebe.default_binary()

    if verbose:
        print("b.set_value('ecc', 0.5)")
    b.set_value('ecc', 0.5)

    if verbose:
        print(b.filter(qualifier=['ecosw', 'esinw', 'ecc', 'per0']))
    ...
    if verbose:
        print("b.flip_constraint('esinw', solve_for='ecc')")
    b.flip_constraint('esinw', solve_for='ecc')

    if verbose:
        print(b.filter(qualifier=['ecosw', 'esinw', 'ecc', 'per0']))

    assert not b.run_checks().passed
    with pytest.raises(ValueError):
        b.run_compute()

    if verbose:
        print("b.set_value('per0', 10)")
    b.set_value('per0', 10)
    ...
    if verbose:
        print("b.flip_constraint('ecosw', solve_for='per0')")
...
def test_pot_filloutfactor(verbose=False):
    if verbose:
        print("b=phoebe.default_binary(contact_binary=True)")
...
if __name__ == '__main__':
    ...
    test_esinw_ecosw(verbose=True)
    test_pot_filloutfactor(verbose=True)
