...
def emcee(**kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [FloatParameter(qualifier='thin_factor', value=kwargs.get('thin_factor', 0.5), default_unit=u.dimensionless_unscaled, limits=(0.001, 1000), description='factor of min(autocorr_times) to apply for the default solution thinning (thinning not applied until adopting the solution)')]
    params += [FloatParameter(qualifier='nlags_factor', value=kwargs.get('nlags_factor', 3), default_unit=u.dimensionless_unscaled, limits=(0.001, 1000), description='factor of max(autocorr_times) to apply for the default solution nlags (nlags only applied when accessing/plotting the autocorrelation function, will not default to larger than niters-burnin)')]
