...
def lc_periodogram(**kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [FloatArrayParameter(visible_if='algorithm:bls', qualifier='duration', value=kwargs.get('duration', _geomspace(0.01,0.3,5) if StrictVersion(np.__version__) >= StrictVersion("1.13") else _linspace(0.01, 0.3, 5)), default_unit=u.dimensionless_unscaled, description='The set of durations (in phase-space) that will be considered.  See https://docs.astropy.org/en/stable/api/astropy.timeseries.BoxLeastSquares.html')]
...
def lc_geometry(**kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [BoolParameter(qualifier='phase_bin', value=kwargs.get('phase_bin', True), description='Bin the input observations (see phase_nbins) if more than 2*phase_nbins.  NOTE: input observational sigmas will be ignored during binning.')]
