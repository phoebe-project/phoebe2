...
def lc_syn(syn=True, **kwargs):
    ...
    if not syn:
        ...
        # syn_params += [FloatArrayParameter(qualifier='phases', value=_empty_array(kwargs, 'phases'), default_unit=u.cycle, description='Observed phases')]
        # syn_params += [ChoiceParameter(qualifier='phase_component', value='', choices=[''], description='Component to use for phases<->times')]
...
def lc_dep(is_lc=True, **kwargs):
    ...
    if is_lc:
        dep_params += [ChoiceParameter(qualifier='pblum_mode', value=kwargs.get('pblum_mode', 'provided'), choices=['provided', 'color coupled', 'system flux', 'total flux', 'scale to data', 'absolute'], description='Mode for scaling passband luminosities')]
        ...
        dep_params += [FloatParameter(visible_if='l3_units:flux', qualifier='l3', value=kwargs.get('l3', 0.), limits=[0, None], default_unit=u.W/u.m**2, description='Third light in flux units')]
...
def lp_syn(syn=True, **kwargs):
    ...
    for time in times:
        ...
        if not syn:
            ...
            # syn_params += [FloatArrayParameter(qualifier='compute_wavelengths', value=kwargs.get('compute_wavelengths', []), default_unit=u.nm, description='Wwavelengths to use during run_compute.  If empty, will use times parameter')]
