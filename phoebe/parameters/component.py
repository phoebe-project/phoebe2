...
def star(component, **kwargs):
    ...
    * `hefrac` (float, optional): helium fraction, log(He/H) abundance by number.
        Not applicable for all atmosphere models.
    ...
    * (<phoebe.parameters.ParameterSet>, list): ParameterSet of all newly created
    ...
    params += [FloatParameter(qualifier='abun', visible_if='hierarchy.is_contact_binary:False,atm_in_computes:!tmap*', value=kwargs.get('abun', 0.), default_unit=u.dimensionless_unscaled, description='Abundance/Metallicity')]   # TODO: correct units??? check if log or not? (logabun = 0)
    params += [FloatParameter(qualifier='hefrac', visible_if='hierarchy.is_contact_binary:False,atm_in_computes:tmap*', value=kwargs.get('hefrac', 0.), default_unit=u.dimensionless_unscaled, limits=(0.0,1.0), description='Helium fraction, log(He/H) abundance by number.')]
