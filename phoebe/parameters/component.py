...
def envelope(component, **kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>, list): ParameterSet of all newly created
    ...
    params += [BoolParameter(qualifier='smoothing_enabled', latexfmt=r'\mathrm{{wenabled}}_\mathrm{{ {component} }}', value=kwargs.get('smoothing_enabled', True), description='Whether to smooth the envelope temperature distribution across the neck')]
    params += [FloatParameter(qualifier='smoothing_factor', latexfmt=r'\mathrm{{w}}_\mathrm{{ {component} }}', value=kwargs.get('smoothing_factor', 0.5), default_unit=u.dimensionless_unscaled, limits=(0.0,1.0), description='Factor for temperature smoothing of the envelope')]
    params += [ChoiceParameter(qualifier='smoothing_method', latexfmt=r'\mathrm{{smethod}}_\mathrm{{ {component} }}', value=kwargs.get('smoothing_method', 'lateral'), choices=['lateral', 'isotropic', 'spotty', 'gaussian'], description='Method for temperature smoothing of the envelope')]
