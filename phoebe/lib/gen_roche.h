...
// #define DEBUG
  ...
  */
  ...
  template <class T, int N> struct glq {
    ...
    static const T c[];         // cos()^2 of Gauss-Lagrange nodes x_i in [0, Pi/2]
    static const T d[];
  ...
  */
  template <>
  const long double glq<long double, 15>::c[]={0.999911065396171632736007253518L,0.997574886997681325232904530099L,0.985854212895255771578898079594L,0.953879938591329487419660590834L,0.890692147886632646280933330332L,0.790164039217058712341619458721L,0.655400162984313217687403793532L,0.5L,0.344599837015686782312596206468L,0.209835960782941287658380541279L,0.109307852113367353719066669668L,0.046120061408670512580339409166L,0.014145787104744228421101920406L,0.0024251130023186747670954699007L,0.0000889346038283672639927464819L};
  template <>
  const long double glq<long double, 15>::d[]={0.0000889266944646091553555375073L,0.0024192318292446596704491832554L,0.013945683811931480320682058875L,0.04399300134433097347511477941L,0.097359645579729565862303734566L,0.165804830345241213606518615509L,0.225850789344448888056399859737L,0.25L,0.225850789344448888056399859737L,0.165804830345241213606518615509L,0.097359645579729565862303734566L,0.04399300134433097347511477941L,0.013945683811931480320682058875L,0.0024192318292446596704491832554L,0.0000889266944646091553555375073L};
  template <>
...

      {
        ...
        for (int i = 0; i < G::n; ++i) {
          ...
          if (b_area) {
            ...
            k[0][G::n] += w[i]*std::sqrt(s*(1 + g*g*G::d[i]) + f*f); // = dA
          }

          if (b_vol)  k[0][G::n+1] += w[i]*s;  // = 2*dV
          if (b_dvol) k[0][G::n+2] += w[i]/(-f1 - q*f2 + b*G::c[i]); // d(dV/dOmega)/dt
        }
        ...
        for (int i = 0; i < G::n; ++i){
          ...
          if (b_area) {
            g *= b; // = -(dR/dc)/R   Note: dR/dphi = -dR/dc*2*sqrt(c(1-c))
            k[1][G::n] += w[i]*std::sqrt(s*(1 + g*g*G::d[i]) + f*f);  // = dA
          }

          if (b_vol) k[1][G::n+1] += w[i]*s; // = 2*dV
          if (b_dvol) k[1][G::n+2] += w[i]/(-f1 - q*f2 + b*G::c[i]); // d(dV/dOmega)/dt
        }
        ...
        for (int i = 0; i < G::n; ++i) {
          ...
          if (b_area) {
            g *= b; // = -(dR/dc)/R   Note: dR/dphi = -dR/dc*2*sqrt(c(1-c))
            k[2][G::n] += w[i]*std::sqrt(s*(1 + g*g*G::d[i]) + f*f); // = dA
          }
          if (b_vol) k[2][G::n+1] += w[i]*s; // = 2*dV
          if (b_dvol) k[2][G::n+2] += w[i]/(-f1 - q*f2 + b*G::c[i]); // d(dV/dOmega)/dt
        }
        ...
        for (int i = 0; i < G::n; ++i){
          ...
          if (b_area) {
            g *= b; // = (dR/dc)/R   Note: dR/dphi = dR/dc*2*sqrt(c(1-c))
            k[3][G::n] += w[i]*std::sqrt(s*(1 + g*g*G::d[i]) + f*f); // = dA
          }

          if (b_vol) k[3][G::n+1] += w[i]*s; // = 2*dV
          if (b_dvol) k[3][G::n+2] += w[i]/(-f1 - q*f2 + b*G::c[i]); // d(dV/dOmega)/dt
        }
      ...
      if (polish){
        ...
        for (int i = 0; i < G::n; ++i) {
          ...
          do {
            ...
            df = -(b*G::c[i] - f1 - q*f2)/2;                     // = dF/dR
            f = omega - q*(g2 - t) - g1 - b*(G::c[i]*s + s1)/2;  // =F
