  ...
  */
  ...
  void calc_dOmega2_pole(
    ...
    T A[2] = {p[1], p[0]*sc_phi[1]},
     ...
     v[0] = (-Wnr-std::sqrt(Wnr*Wnr-Wrr*Wnn))/Wrr;    // dr/dnu
     v[1] = 1/(Wrr*v[0] + Wnr);                       // d^2(dV/dOmega)/(dnu dphi)
  }
  ...
  */
    ...
    const int & m = 1 << 14)
  {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      // 1. step
      if (i == 0) {   // discussing pole = L1 separately
        sum[1] = 0;
        for (int j = 0; j < glq_n; ++j){
          r[0] = y[j], r[1] = r[0]*r[0];
          calc_dOmega2_pole(W, r, glq_sc_phi + 2*j, q, b, p);
          k[0][j] = dnu*W[0];
...
/*
  ...
  */ 
  ...
  bool critical_volume(
    ...
    critial_volume_integration(v[0], x, q, F, d, th, 1 << 10);
    critial_volume_integration(v[1], x, q, F, d, th, 1 << 11);
    
    // extrapolation based on the expansion
    //  I = I0 + a1 h^4 + a2 h^5 + ...
    // which should a valid approximation for RK4 integration 
    // of well-behaved functions     
    
    for (int i = 0; i < 2; ++i)
      volC[i] = (16*v[1][i] - v[0][i])/15;
