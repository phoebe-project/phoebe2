...
template<class T>
    ...
    const int & m = 1 << 14)
  {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      for (int j = 0; j < glq_n; ++j){
        ...
        r[0] = y[j], r[1] = r[0]*r[0];
        calc_dOmega(W, mask, r, sc_nu, glq_sc_phi+2*j, q, b, sc_th);
  ...
  */
    ...
    const int & m = 1 << 14)
  {
    ...
    T w[glq_n], y[dim], k[4][dim], sc_nu[2], p[4], W[3], sum[3],
      r[2], nu, rt, rp,
      d2 = d*d, d3 = d2*d,
      b = (1 + q)*F*F*d3,
      dnu = utils::m_pi/m;
    ...
    for (int i = 0; i < m; ++i) {
      ...
      if (i == 0) {   // discussing pole = L1 separately, sin(nu) = 0
        ...
        for (int j = 0; j < glq_n; ++j){
          r[0] = y[j], r[1] = r[0]*r[0];
          calc_dOmega2_pole(W, mask2, r, glq_sc_phi + 2*j, q, b, p);
          k[0][j] = dnu*W[0];

          if (b_dvol) sum[2] += w[j]*r[1]*W[1];
        }

        if (b_area) k[0][glq_n] = 0;
        if (b_vol) k[0][glq_n+1] = 0;
        if (b_dvol) k[0][glq_n+2] = sum[2];
        ...
        for (int j = 0; j < glq_n; ++j){
          ...
          calc_dOmega2(W, mask, r, sc_nu, glq_sc_phi + 2*j, q, b, p);
        ...
        if (b_area) k[0][glq_n] = sum[0];
        if (b_vol)  k[0][glq_n+1] = sum[1]*sc_nu[0];
        if (b_dvol) k[0][glq_n+2] = sum[2]*sc_nu[0];
      }
      ...
      if (b_area) k[1][glq_n] = sum[0];
      if (b_vol)  k[1][glq_n+1] = sum[1]*sc_nu[0];
      if (b_dvol) k[1][glq_n+2] = sum[2]*sc_nu[0];
      ...
      if (b_area) k[2][glq_n] = sum[0];
      if (b_vol)  k[2][glq_n+1] = sum[1]*sc_nu[0];
      if (b_dvol) k[2][glq_n+2] = sum[2]*sc_nu[0];
      ...
      if (b_area) k[3][glq_n] = sum[0];
      if (b_vol)  k[3][glq_n+1] = sum[1]*sc_nu[0];
      if (b_dvol) k[3][glq_n+2] = sum[2]*sc_nu[0];
    ...
    if (b_area) v[0] = 2*d2*y[glq_n];
    if (b_vol)  v[1] = 2*d3*y[glq_n + 1]/3;
    if (b_dvol) v[2] = 2*d3*d*y[glq_n + 2];
  }
