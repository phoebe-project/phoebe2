      ...
      for (p[1].i = 0, c[1] = CatT; p[1].i < p[0].i; ++p[1].i, c[1] += 3) {
        ...
        if ((p[0].h = utils::dot3D(n[0], a)) > 0 && 
          ...
          if (p[0].h > tmp && p[1].h > tmp) {
            ...
            for (int k = 0; k < 2; ++k)
              p[k].F *= A[p[k].i]*LDmodels[LDidx[p[k].i]]->F(p[k].h/s);
...
struct Tmat_elem_nbody {
  ...
  T F;                      //  F_{ (b_2, i_2)->(b_1, i_1) }
  Tmat_elem_nbody() {}
  Tmat_elem_nbody(int b1, int i1, int b2, int i2, const T &F) 
  : b1(b1), i1(i1), b2(b2), i2(i2), F(F) {}
};


/*
  Solving the Wilson's limb-darkened radiosity problem defined as 
  discusssing emitted and received-reflected flux per TRIANGLE for 
  a set of n convex bodies.
...
template <class T>
void triangle_mesh_radiosity_wilson_triangles_nbody_convex(
  ...
  int nb = LDmodels.size();  // number of bodies
      ...
      for (int j1 = 0, mj = Nt[1]; j1 < mj; ++j1) {
        ...
        if ((h[0] = utils::dot3D(n[0], a)) > 0 && 
          ...
          if (h[0] > tmp && h[1] > tmp) {
            ...
            Fmat.emplace_back(1, j1, 0, i1, F*A[0][i1]*LDmodels[0]->F(h[0]/s));
            // F_{(1,j1) -> (0,i1) } = F_{(0, i1), (1,j1)}
            Fmat.emplace_back(0, i1, 1, j1, F*A[1][j1]*LDmodels[1]->F(h[1]/s));
          }
        }
      }
    }
      ...
      for (p[1].i = 0, m[1] = Nt[p[1].b]; p[1].i < m[1]; ++p[1].i) {
        ...
        if ((p[0].h = utils::dot3D(n[0], a)) > 0 && 
          ...
          if (p[0].h > tmp && p[1].h > tmp) {
            ...
            for (int k = 0; k < 2; ++k)
              p[k].F *= A[p[k].b][p[k].i]*LDmodels[p[k].b]->F(p[k].h/s);
  ...
  { 
    ...
    for (auto && B : DF) {  // loop over bodies
      ...
      for (auto && p : B) {    // loop over triangles
        for (auto && q : p) Fmat.emplace_back(q.b, q.i, b, i, q.F);
        ++i;
      }
      ++b;
    }
...
void triangle_mesh_radiosity_wilson_vertices(
  ...
  int i = 0;
  for (auto && p : DF){ 
    for (auto && q : p) Fmat.emplace_back(i, q.i, q.F); // F_{i,q.i=j} = F_{i<-j}
    ++i;
  }
}
...
/*
  Solving the Wilson's limb-darkened radiosity problem defined as 
  discusssing emitted and received-reflected flux per surfaces attached 
  to VERTICES for a set of n convex bodies.
...
template <class T>
void triangle_mesh_radiosity_wilson_vertices_nbody_convex(
  std::vector <std::vector <T3Dpoint<T>>> & V,              // inputs 
  std::vector <std::vector <T3Dpoint<int>>> & Tr,
      ...
      for (int j1 = 0, mj = Nv[1]; j1 < mj; ++j1) {
        ...
        if ((h[0] = utils::dot3D(n[0], a)) > 0 && 
          ...
          if (h[0] > tmp && h[1] > tmp) {
            ...
            // F_{(0, i1) -> (1,j1) } = F_{(1,j1), (0, i1)}
            Fmat.emplace_back(1, j1, 0, i1, F*AatV[0][i1]*LDmodels[0]->F(h[0]/s));
            // F_{(1,j1) -> (0,i1) } = F_{(0, i1), (1,j1)}
            Fmat.emplace_back(0, i1, 1, j1, F*AatV[1][j1]*LDmodels[1]->F(h[1]/s));
          }
        }
      }
    }
      ...
      for (p[1].i = 0, m[1] = Nv[p[1].b]; p[1].i < m[1]; ++p[1].i) {
        ...
        if ((p[0].h = utils::dot3D(n[0], a)) > 0 && 
          ...
          if (p[0].h > tmp && p[1].h > tmp) {
            ...
            // looking at Tj from Ti and vice versa
            for (int k = 0; k < 2; ++k)
              p[k].F *= AatV[p[k].b][p[k].i]*LDmodels[p[k].b]->F(p[k].h/s);
...
/*
  Solving the radiosity equation for n-body case
...
*/
template <class T>
bool solve_radiosity_equation_nbody(
