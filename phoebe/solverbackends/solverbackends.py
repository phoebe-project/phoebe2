...
class EmceeBackend(BaseSolverBackend):
    ...
    * <phoebe.frontend.bundle.Bundle.run_solver>
    ...
    def _get_packet_and_solution(self, b, solver, **kwargs):
        ...
        solution_params += [_parameters.ChoiceParameter(qualifier='distributions_convert', value='mvsamples', choices=['mvsamples', 'mvhistogram', 'mvgaussian'], description='type of multivariate distribution to use when calling adopt_solution, get_distribution_collection, or plot. mvsamples: chains are stored directly and used for sampling, binned on-the-fly according to distributions_bins to compute probabilities.  mvhistogram: chains are binned according to distributions_bins and stored as an n-dimensional histogram.  mvgaussian: a multivariate gaussian is fitted to the samples, use only if distribution is sufficiently represented by gaussians.')]
