...
#__author__ = "Paul Boley, Michel Hillen"
#__email__ = "boley@mpia-hd.mpg.de, Michel.Hillen@ster.kuleuven.be"
#__date__ ='15 March 2013'
__version__ = '0.4'
_mjdzero = datetime.datetime(1858, 11, 17)
_maxtimediff = datetime.timedelta(seconds=5)
...
class OI_TARGET:
    ...
    def __init__(self, target, raep0, decep0, equinox=2000.0, ra_err=0.0, dec_err=0.0,
                 sysvel=0.0, veltyp='UNKNOWN', veldef='OPTICAL', pmra=0.0, pmdec=0.0,
...
class OI_VIS:
    ...
    def __eq__(self, other):
        ...
        return not (
            (abs(self.timeobs - other.timeobs) > _maxtimediff) or
...
class oifits:
    ...
    def __init__(self):
        ...
        self.wavelength = OrderedDict()
        self.target = np.empty(0)
        self.array = OrderedDict()
        self.vis = np.empty(0)
        self.vis2 = np.empty(0)
        self.t3 = np.empty(0)
        self._allvis = np.zeros(1,dtype=[('visamp','f8'),('visamperr','f8'),('visphi','f8'),('visphierr','f8'),('eff_wave','f8'),('eff_band','f8'),('ucoord','f8'),('vcoord','f8'),('mjd','f8')]).view(ma.MaskedArray)
        self._allvis2 = np.zeros(1,dtype=[('vis2data','f8'),('vis2err','f8'),('eff_wave','f8'),('eff_band','f8'),('ucoord','f8'),('vcoord','f8'),('mjd','f8')]).view(ma.MaskedArray)
        self._allt3 = np.zeros(1,dtype=[('t3amp','f8'),('t3amperr','f8'),('t3phi','f8'),('t3phierr','f8'),('eff_wave','f8'),('eff_band','f8'),('u1coord','f8'),('v1coord','f8'),('u2coord','f8'),('v2coord','f8'),('mjd','f8')]).view(ma.MaskedArray)   
    ...
    def __add__(self, other):
        ...
        if len(other.array):
            stationmap = OrderedDict()
            arraymap = OrderedDict()
    ...
    def __getattr__(self,attrname):
        ...
        if attrname in ('vis','vis2','t3','array','station','wavelength','target'):
            return self.__dict__[attrname]
        elif attrname == 'allvis2':
            self._allvis2 = self._allvis2[:1]
            temp = self._allvis2.copy()
            allmask = [0]
            for i in range(len(self.vis2)):
                # extract the actual observables
                vis2data = getattr(self.vis2[i],'vis2data')
                vis2err = getattr(self.vis2[i],'vis2err')
                wavelength = getattr(self.vis2[i],'wavelength')
                ucoord = getattr(self.vis2[i],'ucoord')
                vcoord = getattr(self.vis2[i],'vcoord')
                timeobs = getattr(self.vis2[i],'timeobs')
                
                # convert the timeobs to MJD
                JD = CD2JD((timeobs.year,timeobs.month,timeobs.day,timeobs.hour,timeobs.minute,timeobs.second+timeobs.microsecond*1e-6))
                mjd = JD - 2400000.5
                
                # return only the valid datapoints
                mask = vis2data.mask
                
                # fill the allvis2 array
                for j in range(len(vis2data.data)):
                    temp[0] = (vis2data.data[j],vis2err.data[j],wavelength.eff_wave[j],wavelength.eff_band[j],ucoord,vcoord,mjd)
                    allmask.append(mask[j])
                    self._allvis2 = np.append(self._allvis2,temp)
            
            self._allvis2.mask = allmask
            return copy.deepcopy(self._allvis2[1:])
        elif attrname == 'allvis':
            self._allvis = self._allvis[:1]
            temp = self._allvis.copy()
            allmask = [0]
            for i in range(len(self.vis)):
                # extract the actual observables
                visampdata = getattr(self.vis[i],'visamp')
                visamperr = getattr(self.vis[i],'visamperr')
                visphidata = getattr(self.vis[i],'visphi')
                visphierr = getattr(self.vis[i],'visphierr')
                wavelength = getattr(self.vis[i],'wavelength')
                ucoord = getattr(self.vis[i],'ucoord')
                vcoord = getattr(self.vis[i],'vcoord')
                timeobs = getattr(self.vis[i],'timeobs')
                ...
                # return only the valid datapoints
                mask = visampdata.mask
                            
                # fill the allvisamp array
                for j in range(len(visampdata.data)):
                    temp[0] = (visampdata.data[j],visamperr.data[j],visphidata.data[j],visphierr.data[j],wavelength.eff_wave[j],wavelength.eff_band[j],ucoord,vcoord,mjd)
                    allmask.append(mask[j])
                    self._allvis = np.append(self._allvis,temp)
            
            self._allvis.mask = allmask    
            return copy.deepcopy(self._allvis[1:])
        elif attrname == 'allt3':
            self._allt3 = self._allt3[:1]
            temp = self._allt3.copy()
            allmask = [0]
            for i in range(len(self.t3)):
                # extract the actual observables
                t3amp = getattr(self.t3[i],'t3amp')
                t3amperr = getattr(self.t3[i],'t3amperr')
                t3phi = getattr(self.t3[i],'t3phi')
                t3phierr = getattr(self.t3[i],'t3phierr')
                wavelength = getattr(self.t3[i],'wavelength')
                u1coord = getattr(self.t3[i],'u1coord')
                v1coord = getattr(self.t3[i],'v1coord')
                u2coord = getattr(self.t3[i],'u2coord')
                v2coord = getattr(self.t3[i],'v2coord')
                timeobs = getattr(self.t3[i],'timeobs')
                ...
                # fill the allvisphi array
                for j in range(len(t3phi.data)):
                    temp[0] = (t3amp.data[j],t3amperr.data[j],t3phi.data[j],t3phierr.data[j],wavelength.eff_wave[j],wavelength.eff_band[j],u1coord,v1coord,u2coord,v2coord,mjd)
                    allmask.append(mask[j])
                    self._allt3 = np.append(self._allt3,temp)
            
            self._allt3.mask = allmask 
            return copy.deepcopy(self._allt3[1:])
        else:
            raise AttributeError(attrname)
        
    def setAllAttr(self,attrname,value):
        """
        This function should only be used to replace the actual measurements (i.e. visamp, visphi, vis2, t3amp or t3phi) with a new value, 
        so nothing is changed to the structure of the data set. All other 'independent' variables (i.e. u,v,t,wavelength) 
        can not be changed in this way, even they are present in the 'value' array. For those, the original data 
        attributes should be adressed directly because there might be repercussions for other variables. 
        NOTE: value should be an array that is exactly equal to self.allvis, self.allvis2 or self.allt3, except for the values in the columns visamp,
        visphi, vis2, t3amp and t3phi, and their corresponding errors which are currently ignored.
        NOTE: BEWARE that this function changes the underlying object (which includes copies of this object, unless deepcopy is used!).
        """
        # first check if the basic input is correct
        original = self.__getattr__(attrname)
        if attrname in ('vis','vis2','t3','array','station','wavelength','target'):
            raise NotImplementedError
        elif attrname in ('allvis','allvis2','allt3'):
            if type(original) != type(value): 
                raise TypeError("New value not of the same type!")
            elif original.ndim != value.ndim:
                raise ValueError("New value does not have the same number of dimensions!")
            elif original.shape != value.shape:
                raise ValueError("New value does not have the same shape!")
            elif (
                (original['mjd'] != value['mjd']).any() or
                (original['eff_wave'] != value['eff_wave']).any() or
                (original['eff_band'] != value['eff_band']).any()):
                    raise ValueError("New values of mjd, eff_wave and eff_band need to be the same as the stored ones!")
        else:
            raise AttributeError(attrname)
        
        if attrname == 'allvis':
            if (
                (original['ucoord'] != value['ucoord']).any() or
                (original['vcoord'] != value['vcoord']).any()):
                    raise ValueError("New values of ucoord and vcoord need to be the same as the stored ones!")
            else:
                N = len(value)
                index0 = 0
                indexn = 0
                for i in range(len(self.vis)):
                    data = getattr(self.vis[i],'visamp')
                    indexn = index0 + len(data)
                    setattr(self.vis[i],'visamp',np.array(value.data['visamp'][index0:indexn],dtype=double).reshape(-1))
                    setattr(self.vis[i],'visphi',np.array(value.data['visphi'][index0:indexn],dtype=double).reshape(-1))
                    index0 = indexn       
        ...
        if attrname == 'allvis2':
            if (
                ...
                indexn = 0
                for i in range(len(self.vis2)):
                    data = getattr(self.vis2[i],'vis2data')
                    indexn = index0 + len(data)
                    setattr(self.vis2[i],'vis2data',np.array(value['vis2data'][index0:indexn],dtype=double).reshape(-1))
                    index0 = indexn
        
        if attrname == 'allt3':
            if (
                 (original['u1coord'] != value['u1coord']).any() or
                 (original['v1coord'] != value['v1coord']).any() or
                 (original['u2coord'] != value['u2coord']).any() or  
                 (original['v2coord'] != value['v2coord']).any()):
                     raise ValueError("New values of u1coord, v1coord, u2coord and v2coord need to be the same as the stored ones!")
            else:
                N = len(value)
                index0 = 0
                indexn = 0
                for i in range(len(self.t3)):
                    data = getattr(self.t3[i],'t3phi')
                    indexn = index0 + len(data)
                    setattr(self.t3[i],'t3amp',np.array(value.data['t3amp'][index0:indexn],dtype=double).reshape(-1))
                    setattr(self.t3[i],'t3phi',np.array(value.data['t3phi'][index0:indexn],dtype=double).reshape(-1))
                    index0 = indexn    
    ...
    def save(self, filename, hdcomment = None):
        ...
        if hdcomment:
            hdu.header.add_comment(hdcomment)
        ...
        if self.vis.size:
            ...
            for vis in self.vis:
                ...
                if vis.timeobs:
                    timeobs = vis.timeobs
                    time = timeobs - refdate
                    data['time'].append(time.days * 24.0 * 3600.0 + time.seconds)
                    mjd = CD2JD((timeobs.year,timeobs.month,timeobs.day,timeobs.hour,timeobs.minute,timeobs.second+timeobs.microsecond*1e-6)) - 2400000.5
                    data['mjd'].append(mjd)
                else:
...
def open(filename, quiet=False):
    ...
    for hdu in hdulist:
        ...
        if hdu.name == 'OI_WAVELENGTH':
            if newobj.wavelength == None: newobj.wavelength = OrderedDict()
            ...
            if newobj.array == None: newobj.array = OrderedDict()
    ...
    for hdu in hdulist:
        ...
        if hdu.name == 'OI_VIS':
            for row in data:
                ...
                date = JD2CD(2400000.5+row['MJD'])
                timeobs = datetime.datetime(int(date[0]), int(date[1]), int(date[2])) + datetime.timedelta(days=np.fmod(date[2],1.))
...
def oifits2vis(fits_file):
    templatedata = open(oifits)
    allvis2 = templatedata.allvis2 

    allvis2['vis2data'] = np.sqrt(allvis2['vis2data'])
    allvis2['vis2err'] = allvis2['vis2err']/(2.0*allvis2['vis2data'])

    np.savetxt('pionier.vis',allvis2[['ucoord','vcoord','vis2data','vis2err','mjd']])