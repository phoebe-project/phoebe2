      subroutine lcnotemp(mpagexxx,nrefxxx,mrefxxx,ifsmv1xxx,ifsmv2xxx,
     + icor1xxx,icor2xxx,
     + ldxxx,jdphsxxx,hjd0xxx,periodxxx,dpdt,pshxxx,stdev,noise,seed,
     + hjdst,hjdsp,
     + hjdin,phstrt,phstop,phin,phn,mode,ipbxxx,ifat1,ifat2,n1,n2,
     + perr0xxx,
     + dperdtxxx,the,vunitxxx,exxx,axxx,f1,f2,vgaxxx,xincl,gr1,gr2,
     + abunin, tavh,tavc,
     + alb1,alb2,poth,potc,rm,xbol1,xbol2,ybol1,ybol2,iband,hlum,
     + clum,xh,xc,yh,yc,el3,opsfxxx,zero,factor,wl,xlat,xlong,
     + radsp,temsp,
     + atmtab,pltab,request,vertno,L3perc,indeps,
     + depsflux,depsvkm1,depsvkm2,skycoy,skycoz,params)


Cf2py intent(in) MPAGExxx
Cf2py intent(in) NREFxxx
Cf2py intent(in) MREFxxx
Cf2py intent(in) IFSMV1xxx
Cf2py intent(in) IFSMV2xxx
Cf2py intent(in) ICOR1xxx
Cf2py intent(in) ICOR2xxx
Cf2py intent(in) LDxxx
Cf2py intent(in) JDPHSxxx
Cf2py intent(in) HJD0xxx
Cf2py intent(in) PERIODxxx
Cf2py intent(in) DPDT
Cf2py intent(in) pshxxx
Cf2py intent(in) STDEV
Cf2py intent(in) NOISE
Cf2py intent(in) SEED
Cf2py intent(in) hjdst
Cf2py intent(in) hjdsp
Cf2py intent(in) hjdin
Cf2py intent(in) PHSTRT
Cf2py intent(in) phstop
Cf2py intent(in) PHIN
Cf2py intent(in) PHN
Cf2py intent(in) MODE
Cf2py intent(in) IPBxxx
Cf2py intent(in) IFAT1
Cf2py intent(in) IFAT2
Cf2py intent(in) N1
Cf2py intent(in) N2
Cf2py intent(in) PERR0xxx
Cf2py intent(in) DPERDTxxx
Cf2py intent(in) THE
Cf2py intent(in) VUNITxxx
Cf2py intent(in) Exxx
Cf2py intent(in) Axxx
Cf2py intent(in) F1
Cf2py intent(in) F2
Cf2py intent(in) VGAxxx
...
Cf2py intent(in) poth
Cf2py intent(in) potc
Cf2py intent(in) RM
...
Cf2py intent(in) HLUM
Cf2py intent(in) CLUM
Cf2py intent(in) xh
Cf2py intent(in) xc
Cf2py intent(in) yh
Cf2py intent(in) yc
Cf2py intent(in) EL3
Cf2py intent(in) OPSFxxx
Cf2py intent(in) zero
Cf2py intent(in) FACTOR
Cf2py intent(in) wl
Cf2py intent(in) XLAT
Cf2py intent(in) XLONG
Cf2py intent(in) RADSP
Cf2py intent(in) TEMSP
Cf2py intent(in) atmtab
Cf2py intent(in) pltab
Cf2py intent(in) request
Cf2py intent(in) vertno
Cf2py intent(in) L3perc
Cf2py intent(in,out) indeps
Cf2py intent(in,out) depsflux
Cf2py intent(in,out) depsvkm1
Cf2py intent(in,out) depsvkm2
Cf2py intent(in,out) skycoy
Cf2py intent(in,out) skycoz
Cf2py intent(in,out) params
...
c
      ...
      double precision indeps(*),skycoy(*),skycoz(*),params(*)
      double precision depsflux(*),depsvkm1(*),depsvkm2(*)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc


      dimension wll1(1),ewid1(1),depth1(1),wll2(1),ewid2(1),depth2(1)
      dimension xlat(2,*),xlong(2,*),radsp(2,*),temsp(2,*),kks(2,1)
      dimension xcl(1),ycl(1),zcl(1),rcl(1),op1(1),fcl(1),edens(1),
     $          xmue(1),encl(1)
     ...
     $gmag2(igsmax),glog1(igsmax),glog2(igsmax)
      dimension dvks1(lpimax),dvks2(lpimax),
     $tau1(lpimax),tau2(lpimax),emm1(lpimax),emm2(lpimax),
     $hbarw1(lpimax),
     $hbarw2(lpimax)
     ...
     $clight
      ...
      radsp2=1
      temsp2=1
c      call wrlci("junk",MPAGE,NREF,MREF,IFSMV1,IFSMV2,ICOR1,ICOR2,LD,
c     +JDPHS,HJD0,PERIOD,DPDT,PSHIFT,STDEV,NOISE,SEED,hjdst,hjdsp,
c     +hjdin,PHSTRT,phstop,phin,phn,MODE,IPB,IFAT1,IFAT2,N1,N2,PERR0,
c     +DPERDT,THE,VUNIT,E,a,F1,F2,VGA,XINCL,GR1,GR2,ABUNIN,TAVH,TAVC,
c     +ALB1,ALB2,poth,potc,RM,XBOL1,XBOL2,YBOL1,YBOL2,IBAND,hlum,clum,xh,
c     +xc,yh,yc,EL3,OPSF,zero,FACTOR,wl,NSP1,XLAT1,XLONG1,RADSP1,
c     +TEMSP1,NSP2,XLAT2,XLONG2,RADSP2,TEMSP2)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      close(22)
      close(23)
      open(unit=16,file='/dev/null')
      ibef=0
      nf1=1
      nf2=1
      DO 1000 IT=1,1000
      KH=17
c      if(mpage.ne.9) goto 414
      if(it.gt.1) then
      close(16)
      return
      endif
