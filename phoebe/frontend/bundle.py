            ...
            for compute in computes:
                ...
                for ds in model_ps.datasets:
                    ...
                    if len(gp_features) > 0:
                        ...
                        # TODO: update checks to check for celerite2
                        # if not _use_sklearn:
                        ...
                        sklearn_gp_kernels = []
                        sklearn_alg_operations = []
                        celerite2_gp_kernels = [] 
                        celerite2_alg_operations = []
                        ...
                        for gp in gp_features:
                            ...
                            if kind in ['sho', 'rotation', 'matern32']:
                                celerite2_alg_operations.append(kwargs.pop('alg_operation'))
                                celerite2_gp_kernels.append(gp_kernel_classes.get(kind)(**kwargs))
                            else:
                                sklearn_alg_operations.append(kwargs.pop('alg_operation'))
                                sklearn_gp_kernels.append(gp_kernel_classes.get(kind)(**kwargs))

                            # TODO: figure out how to handle this per gp_feature or as an external parameter visible only if gp features present
                            # this way only takes the value from the last feature
                        ...
                        if len(sklearn_gp_kernels) > 0:
                            ...
                            if len(sklearn_gp_kernels) > 1:
                                for i in range(1, len(sklearn_gp_kernels)):
                                    if sklearn_alg_operations[i] == 'product':
                                        sklearn_gp_kernel *= sklearn_gp_kernels[i]
                                        # print(gp_kernel)
                                    else:
                                        sklearn_gp_kernel += sklearn_gp_kernels[i]
                                        # print(gp_kernel)

                            gp_regressor = GaussianProcessRegressor(kernel=sklearn_gp_kernel)
                        ...
                        if len(celerite2_gp_kernels) > 0:
                            ...
                            if len(celerite2_gp_kernels) > 1:
                                for i in range(1, len(celerite2_gp_kernels)):
                                    if celerite2_alg_operations[i] == 'product':
                                        celerite2_gp_kernel *= celerite2_gp_kernels[i]
                                        # print(gp_kernel)
                                    else:
                                        celerite2_gp_kernel += celerite2_gp_kernels[i]
                                        # print(gp_kernel)
