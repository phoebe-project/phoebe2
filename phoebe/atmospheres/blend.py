...
ph_axes = pb._phoenix_axes
ck_ints = pb._ck2004_energy_grid
ph_ints = pb._phoenix_energy_grid

new_axes = (
    np.concatenate((ph_axes[0], np.arange(12250., 13001, 250), ck_axes[0][39:])),
    ph_axes[1],
    ph_axes[2]
)

new_table, extrapolant = extrapolate(new_axes, ph_axes, ph_ints)
ck_table, ck_extrapolant = extrapolate(new_axes, ck_axes, ck_ints)

for xi, x in enumerate(new_axes[0]):
    for yi, y in enumerate(new_axes[1]):
        if np.isnan(new_table[xi, yi, 6, 0]):
            continue
        plt.plot(x, y, 'go')

for xi, x in enumerate(ph_axes[0]):
    for yi, y in enumerate(ph_axes[1]):
        if np.isnan(ph_ints[xi, yi, 6, 0]):
            continue
        plt.plot(x, y+0.05, 'bo')

for xi, x in enumerate(ck_axes[0]):
    for yi, y in enumerate(ck_axes[1]):
        if np.isnan(ck_ints[xi, yi, 5, 0]):
            continue
        plt.plot(x, y+0.1, 'ro')

for xi, x in enumerate(new_axes[0]):
    for yi, y in enumerate(new_axes[1]):
        if np.isnan(extrapolant[xi, yi, 6, 0]):
            continue
        plt.plot(x, y+0.15, 'mo')

# for xi, x in enumerate(new_axes[0]):
#     for yi, y in enumerate(new_axes[1]):
#         if np.isnan(ck_extrapolant[xi, yi, 6, 0]):
#             continue
#         plt.plot(x, y+0.15, 'yo')

plt.show()


# blend the edge:
ph_edge = edge(new_axes, new_table)
blend = ph_edge * 0.5 + ck_extrapolant * 0.5
