...
//    gtk_tree_view_insert_column ((GtkTreeView*)phoebe_fitt_curve_treeview, column, -1);
    ...
    renderer    = gtk_cell_renderer_text_new ();
    column      = gtk_tree_view_column_new_with_attributes("Relative weight", renderer, "text", CURVE_COL_NEWCHI2, NULL);
    gtk_tree_view_insert_column ((GtkTreeView*)phoebe_fitt_curve_treeview, column, -1);
...
int gui_data_lc_treeview_add ()
{
    ...
    switch (result){
        case GTK_RESPONSE_OK:{
			...
			gchar *id;
			id = (gchar *)strdup(gtk_entry_get_text((GtkEntry*)phoebe_load_lc_id_entry));
