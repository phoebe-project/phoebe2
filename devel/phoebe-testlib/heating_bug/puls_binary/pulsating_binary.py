...
pulsating_star = phoebe.BinaryStar(pstar,orbit,mesh1,pbdep=[lcdep1])
...
for kwargs, name in zip([dict(), dict(heating=True), dict(heating=True, refl=True)], ['nothing', 'heating', 'reflection']):
  phoebe.observe(system,time,subdiv_num=3,lc=True, eclipse_alg='binary', **kwargs)#,extra_func=extra_funcs,extra_func_kwargs=extra_funcs_kwargs)
  ...
  #plt.figure()
  plt.plot(times,flux,'-', label=name)
...
plt.savefig('no_puls_comparison_lc.png', dpi=200)     
