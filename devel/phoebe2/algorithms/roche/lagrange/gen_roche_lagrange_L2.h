...
#define __gen_roche_langrange_L2_h
...
#if defined(DEBUG)
double langrange_point_L2_x; // approximation going into N-R iteration
int langrange_point_L2_n;    // number of interations in Newton-Raphson
#endif

template <class T>
T langrange_point_L2(
  const T & q,
  const T & F = 1,
  const T & delta = 1
) {
