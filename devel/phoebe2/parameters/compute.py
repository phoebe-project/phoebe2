...
import phoebe2.dynamics as dynamics
from phoebe2.atmospheres import passbands # needed to get choices for 'atm' parameter
...
def phoebe(**kwargs):
    ...
    Please see :func:`phoebe2.backend.backends.phoebe` for a list of sources to
    ...
    # DYNAMICS
    params += [ChoiceParameter(qualifier='dynamics_method', value=kwargs.get('dynamics_method', 'keplerian'), choices=['keplerian', 'nbody'], description='Which method to use to determine the dynamics of components')]
    ...
    params += [ChoiceParameter(relevant_if='mesh_method:marching', copy_for={'method': ['star', 'envelope'], 'component': '*'}, component='_default', qualifier='distortion_method', value=kwargs.get('distortion_method', 'roche'), choices=['roche', 'nbody', 'sphere'], description='Method to use for distorting stars')]
    ...
    # PER-COMPONENT
    params += [ChoiceParameter(copy_for = {'method': ['star'], 'component': '*'}, component='_default', qualifier='atm', value=kwargs.get('atm', 'blackbody'), choices=_atm_choices, description='Atmosphere table')]
...
def legacy(**kwargs):
    ...
    Please see :func:`phoebe2.backend.backends.legacy` for a list of sources to
    cite when using this backend.
...
def photodynam(**kwargs):
    ...
    Please see :func:`phoebe2.backend.backends.photodynam` for a list of sources to
    cite when using this backend.
...
def jktebop(**kwargs):
    ...
    Please see :func:`phoebe2.backend.backends.jktebop` for a list of sources to
    cite when using this backend.
